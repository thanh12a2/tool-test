...
public class LoginTest extends BaseAppiumTest {
    ...
    public void TC09_loginWithTooLongPhoneNumber() {
        ...
        try {
            WebElement txtPhone = findById("txtPhoneNumber");
            clearAndType(txtPhone, "90123456789012345"); // quá dài

            findById("btnGetVerificationCode").click();
            sleep(3000);

            String activity = driver.currentActivity();
            // Không được crash, phải xử lý lỗi gracefully
            assertNotNull("App không được crash với SĐT quá dài", activity);

            recordResult(tcId + " - SĐT quá dài", "Đang ở LoginActivity", "Nhập SĐT rất dài, nhấn Lấy OTP", "SĐT: 90123456789012345", "Từ chối hoặc cắt bớt SĐT, app không crash", "App xử lý SĐT quá dài không crash", "PASS", "");

        } catch (Throwable e) {
            recordResult(tcId + " - SĐT quá dài", "Đang ở LoginActivity", "Nhập SĐT rất dài, nhấn Lấy OTP", "SĐT: 90123456789012345", "Từ chối hoặc cắt bớt SĐT, app không crash", e.getMessage(), "FAIL", "Có thể crash app");
