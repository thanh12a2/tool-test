...
// Kịch bản A-F-08 - phân bổ cho tìm kiếm bác sĩ:
...
pm.test("A-F-08-TC01 - Tìm kiếm theo tên bác sĩ hợp lệ: hiển thị danh sách bác sĩ với tên, chuyên khoa, phòng", function () {
    if (!isNormalName) {
...
pm.test("A-F-08-TC02 - Tìm kiếm không có kết quả: trạng thái trống", function () {
    if (!isNoMatch) {
...
pm.test("A-F-08-TC03 - Tìm kiếm với chuỗi rỗng: trả về danh sách (không có kết quả hoặc hiển thị tất cả)", function () {
    if (!isEmpty) {
...
pm.test("A-F-08-TC04 - Tìm kiếm với một ký tự: lọc theo ký tự hoặc báo cần nhập thêm", function () {
    if (!isOneChar) {
...
pm.test("A-F-08-TC05 - Tìm kiếm ký tự đặc biệt / XSS: input được làm sạch, không có XSS, kết quả trống", function () {
    if (!isSpecialXss) {
