...
// Kịch bản A-F-06: Xem phác đồ điều trị / đơn thuốc của bệnh nhân
...
// ---------- A-F-06-TC01: Status code & cấu trúc response ----------
pm.test("A-F-06-TC01 - Response trả về 200 và có cấu trúc (result, quantity, data là mảng)", function () {
...
// ---------- A-F-06-TC02: quantity phải khớp với độ dài data ----------
pm.test("A-F-06-TC02 - quantity phải khớp với số lượng phần tử trong data", function () {
    pm.expect(response.quantity, "quantity === data.length").to.eql(data.length);
});

// ---------- A-F-06-TC03: Mỗi thuốc phải có đầy đủ thông tin cần thiết ----------
pm.test("A-F-06-TC03 - Mỗi thuốc có đầy đủ: id, appointment_id, name, type, times, purpose, instruction, repeat_days, repeat_time", function () {
...
// ---------- A-F-06-TC04 (dự kiến FAIL): type phải thuộc danh sách chuẩn ----------
pm.test("A-F-06-TC04 - Loại thuốc (type) phải thuộc danh sách hợp lệ: Uống, Tiêm, Bôi, Ngậm, Xịt, Nhỏ mắt, Đặt", function () {
...
// ---------- A-F-06-TC05 (dự kiến FAIL): repeat_time phải đúng format HH:mm 24h ----------
pm.test("A-F-06-TC05 - repeat_time phải đúng định dạng HH:mm (24h), không dùng AM/PM", function () {
