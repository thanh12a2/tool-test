...
pm.test("A-F-10-TC01 - Cập nhật tên/tuổi/địa chỉ thành công: response OK và dữ liệu được lưu", function (done) {
    const payload = {
        action: "personal",
        name: "mới",
        gender: "Nam",
...
pm.test("A-F-10-TC02 - Upload ảnh đại diện: endpoint nhận request và không trả 5xx", function (done) {
...
pm.test("A-F-10-TC03 - Validation: để trống tên phải báo lỗi 'Tên là bắt buộc'", function (done) {
    const payload = {
        action: "personal",
        name: "",
        gender: "Nam",
...
pm.test("A-F-10-TC04 - Validation: ngày sinh không hợp lệ (tương lai/sai format) phải báo lỗi", function (done) {
    const payload = {
        action: "personal",
        name: "thanhle",
        gender: "Nam",
...
pm.test("A-F-10-TC05 - View profile: trả về đầy đủ thông tin cá nhân (name, gender, birthday, address)", function () {
