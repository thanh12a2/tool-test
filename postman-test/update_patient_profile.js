...
pm.test("A-F-11-TC01 - Cập nhật tên/tuổi/địa chỉ thành công: response OK", function () {
...
pm.test("A-F-11-TC02 - Upload ảnh đại diện: endpoint nhận request và không trả 5xx", function (done) {
...
pm.test("A-F-11-TC03 - Validation: để trống tên phải báo lỗi 'Tên là bắt buộc'", function (done) {
    const payload = {
        action: "personal",
        name: "",
        gender: "Nam",
...
pm.test("A-F-11-TC04 - Validation: ngày sinh không hợp lệ phải báo lỗi", function (done) {
    const payload = {
        action: "personal",
        name: "thanhle",
        gender: "Nam",
...
pm.test("A-F-11-TC05 - View profile sau update: trả về đầy đủ thông tin cá nhân", function (done) {
    pm.sendRequest({
        url: BASE,
        method: "GET",
        header: JSON_HEADERS
